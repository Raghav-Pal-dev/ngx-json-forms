  ...
  template: `
    <div class="card">
      ...
      @if (last(); as e) {
        ...
        <!-- TreeNode emits parent↔children circular refs that crash JsonPipe.
             safeStringify() strips them before pretty-printing. -->
        <pre>{{ safeStringify(e.values) }}</pre>
      }
    </div>
  `,
})
export class TreeSelectComponent {
  ...
  /**
   * PrimeNG's TreeNode keeps `parent` and `children` references that form
   * a cycle. JSON.stringify throws on cycles. Strip them by writing a
   * shallow replacer that skips `parent`. We don't expose the engine
   * value as-is to backend; consumers either send the `key` or pluck the
